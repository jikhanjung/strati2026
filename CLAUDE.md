...
- 프레임워크: **Django 5.2**
- UI: **모바일(스마트폰) 우선** — 발표 목록 탐색 → 북마크 → 내 타임테이블(시간/장소).
- 배포: **Docker 이미지** `honestjung/strati2026` (태그 `0.1.0`부터, `latest` 병행).
  운영 서버의 **`/srv/strati2026`** 디렉토리에서 deploy.
- 데이터: `output/`의 JSON을 production DB(`schema.sql`)로 적재해 앱이 사용.
...
- `output/abstracts.json` — 초록 + 저자/소속/키워드 (중첩 또는 정규화 형태)
- `output/handbook.json` — 위원회/기조연사/일정표/답사 등
...
- [ ] 핸드북 추출·파싱 → `output/handbook.json` (일정표 복호화 + 초록↔시간/장소 매핑)
- [ ] G18·S14 세션 제목 보강
- [ ] `schema.sql` 작성 (production 참고용)
- [ ] Django 5.2 앱 + Docker 이미지(`honestjung/strati2026`) + `/srv/strati2026` 배포
